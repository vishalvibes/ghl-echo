...
- `src/components/ui/*` — small hand-rolled primitives (Card, StatTile, VerdictBadge, Input, Select, AgentSelect, WindowSelect, EmptyState, LoadingBlock).
...
- Form controls go through `Input` / `Select`. `Select` is a Reka UI (shadcn-style) dropdown — pass `options: { value, label }[]`, not native `<option>` slots. Both share the control scale (`h-9 px-3 text-sm rounded-md`); callers add width only.
- **Minimum type size is `text-sm`.** No `text-xs`, no `text-[10px]`. Hierarchy comes from weight and ink level (`text-ink-2` / `text-ink-3`), not from shrinking text.
