 ...
 *
 * Runs ahead of, and independently of, the scorecard judge: a location with no
 * scorecard yet still gets outcome, comprehension, tone and missed
 * opportunities on every call. Same idempotency shape as the judge — a row
 * that already carries `quality` is left alone rather than re-billed.
 */
...
/** Inngest entry point: reload durable rows inside the retryable step. */
